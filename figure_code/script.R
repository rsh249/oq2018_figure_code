...
# v=306
# mai=c(0.4,0.4,0.4,0.4) 
# plot(0.1*(-100:0), doit[[v]][,1], type ='l', ylim=c(-7,5))
# points(0.1*(-500:0), doit[[v]][,1]+1.96*(doit[[v]][,2]/sqrt(doit[[v]][,3])), type ='l',lty=3)
# points(0.1*(-500:0), doit[[v]][,1]-1.96*(doit[[v]][,2]/sqrt(doit[[v]][,3])), type ='l',lty=3)
#  abline(h=0)
# points(0.1*(-500:0), doit[[v+1]][,1], type ='l', col='darkgrey')
# points(0.1*(-500:0), doit[[v+1]][,1]+1.96*(doit[[v+1]][,2]/sqrt(doit[[v+1]][,3])), type ='l',lty=3, col='darkgrey')
# points(0.1*(-500:0), doit[[v+1]][,1]-1.96*(doit[[v+1]][,2]/sqrt(doit[[v+1]][,3])), type ='l',lty=3, col='darkgrey')
...
tiff('figs/Midden_time_series.tif', width = 7.25, height = 6, units='in', res = 450)
m = rbind(c(1,1), c(2,2))
layout(m)
...
plot(0.1*(-500:0), doit[[1]][1:501,3], type = "h", ylab ='', xlab = "kCal. Years BP",  cex.lab =0.9, axes = F, lwd =2, xlim=c(-50, 0), ylim = c(0, max(doit[[1]][,3])))
#hist( doit[[i]][1:50,3], type = "l", ylab ='', xlab = "kCal. Years BP", axes = F, lwd =2, xlim=c(-50, 0), ylim = c(0, max(doit[[i]][,3])))
...
plot((-500:0)/10, doit[[1]][,4], type = "l", ylab = '',xlab = "kCal. Years BP", cex.lab =0.9,axes = F, col='black',  lwd=2, xlim = c(-50, 2), ylim = c(8, 26))
...
insolation <- function(times, astrosol=ber78,...)
  sapply(times, function(tt) Insol(orbit=astrosol(tt), lat = 40*pi/180))
...
tiff('figs/MAT_solar_gisp_comp.tif', 
...
mtext("45°N June insolation [W/m2]", 
      side=4, line=3, cex=0.7,las=0, adj = 0.3)
mtext("Antarctic CO2 Composite (ppm)", 
      side=2, line=3, cex=0.7,las=0, adj = 0)
mtext("kCal. Years BP", side=1, line=3, cex=0.7,las=0)
...
tiff('figs/ALLb.tif', height = 8.5, width = 5, units ='in', res = 450) ##Want: MaximumT.do_timelapse.1, MinimumT.do_timelapse.1, do_time.1.MAP,do_time.1.wbal,WINTERLEN.do_time.1
...
plot(0.1*(-500:0), c(MinT), type="l", lwd = 2.5, lty = 1, axes = F, xlab ='', cex.lab = 0.9, ylab = "Avg. Minimum Temp. (°C)", ylim =c(-4.5,5), panel.first = grid(NULL, 0))
points(0.1*(-500:0), c(MinT)+1.96*MinT.se, type ='l', lty=3)
points(0.1*(-500:0), c(MinT)-1.96*MinT.se, type ='l', lty=3)
abline(h=0)
#text(-15,2.5, "Avg. Minimum Temp. (°C)", pos=c(2), cex =0.8)
axis(side=2, at = pretty(c(max(c(MinT)+1.96*MinT.se), min(c(MinT)-1.96*MinT.se)), n =5),cex.axis = 0.8, tick = TRUE)
textbox(c(-50, -12),4.25,c("Pleistocene"), box = T,cex =0.7, lwd=1.5, adj=c(0,0.7))
textbox(c(-11.5,0),4.25,c("Holocene"), box = T, cex = 0.7, lwd=1.5, adj=c(0,0.7))
textbox(c(-24.5,-16),3.25,c("LGM"), box = T, cex = 0.7, lwd=1.5, adj=c(0,0.7))
textbox(c(-50, -49),2,
        c("A)"), box = F, cex = 1.2)
...
plot(0.1*(-500:0), c(MaxT), type="l", lwd = 2.5, lty = 1, axes = F, xlab ='', cex.lab = 0.9, ylab = "Avg. Maximum Temp. (°C)", ylim = c(-7,3), panel.first = grid(NULL, 0)) # first plot
...
textbox(c(-50, -49),2,
        c("B)"), box = F, cex = 1.2)
#text(-15,1.5, "Avg. Maximum Temp. (°C)", pos=c(2), cex =0.8)

#mtext("MAT (°C)", side = 2, line = 3)



#mtext("Number of contributing midden sites per 100 years", side=4, line=3)
...
plot(0.1*(-500:0), c(MAPT), type="l", lwd = 2.5, lty = 1, axes = F, xlab ='', cex.lab = 0.9, ylab = "Annual Precipitation (mm)", ylim = c(-100, 125), panel.first = grid(NULL, 0)) # first plot
points(0.1*(-500:0), c(MAPT)+1.96*MAPT.se, type ='l', lty=3)
points(0.1*(-500:0), c(MAPT)-1.96*MAPT.se, type ='l', lty=3)
abline(h=0)
#text(-15,-50, "Annual Precipitation (mm)", pos=c(2), cex =0.8)
...
plot(0.1*(-500:0), c(wbal), type="l", lwd = 2.5, lty = 1, axes = F, xlab ='', cex.lab = 0.9, ylab = "Annual Water Balance (mm)", ylim =c(-250, 400), panel.first = grid(NULL, 0)) # first plot
points(0.1*(-500:0), c(wbal)+1.96*wbal.se, type ='l', lty=3)
points(0.1*(-500:0), c(wbal)-1.96*wbal.se, type ='l', lty=3)
abline(h=0)
#text(-15,-90, "Annual Water Balance (mm)", pos=c(2), cex =0.8)

axis(side=2, at = pretty(c(max(c(wbal)+1.96*wbal.se), min(c(wbal)-1.96*wbal.se)), n =5),cex.axis = 0.8, tick = TRUE)
#axis(side = 1, at = pretty(0.1*(-500:0), n = 50), cex = 0.8)
#mtext("kCal. Years BP", side=1, line=3, cex=0.8,las=0)
textbox(c(-50, -49),400,
        c("D)"), box = F, cex = 1.2)
...
plot(0.1*(-500:0), c(win), type="l", lwd = 2.5, 
     ...
     ylab = "Winter Length (months)", ylim = c(-2.5,2.5), panel.first = grid(NULL, 0)) # first plot
points(0.1*(-500:0), c(win)+1.96*win.se, type ='l', lty=3)
points(0.1*(-500:0), c(win)-1.96*win.se, type ='l', lty=3)
#points(0.1*(-500:0), c(MinT), type ='l', lty=1, col = 'red')
abline(h=0)
#text(-10, -1, "Winter Length (months)", pos=c(2), cex =0.8)
axis(side=2, at = pretty(c(max(c(win)+1.96*win.se), min(c(win)-1.96*win.se)), n =5),cex.axis = 0.8, tick = TRUE)
axis(side = 1, at = pretty(0.1*(-500:0), n = 10), cex = 0.7)
mtext("kCal. Years BP", side=1, line=3, cex=0.7,las=0)
textbox(c(-50, -49),2,
        c("E)"), box = F, cex = 1.2)
rect(0.1*inc.inter[,1], -1.5, 0.1*inc.inter[,2], 21, density = -1, col = rgb(0,1,1.0,alpha=0.3), lwd=0)
rect(0.1*dec.inter[,1], -1.5, 0.1*dec.inter[,2], 21, density = -1, col = rgb(0,0,1.0,alpha=0.3), lwd=0)


dev.off()


##Fig 5. LGM projected wbalann (Thornwaite's water balance)
...
library(latticeExtra)
 ...
 p5 = xyplot(diffh ~ m.holo$lat, data.frame(cbind(m.holo$lat, diffh)), main=list("Holocene Regression", cex=0.7), pch = 20, cex = 0.7, xlim = c(30,40), ylim = c(-1000, 500),
             xlab = list("Latitude", cex =0.55), ylab = list("Estimated change in\nwater balance (mm)", cex=0.7), type = c("p", "r"), col = 'black') #+
